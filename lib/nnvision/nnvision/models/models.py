...
from .readouts import MultipleFullGaussian2d, MultiReadout, MultipleSpatialXFeatureLinear, MultipleRemappedGaussian2d, MultipleGaussian2d, MultipleAttention2d, SaliencyShifter
...
def se_core_saliency_shifted_readout(dataloaders, seed, hidden_channels=32, input_kern=13,  # core args
    ...
    if data_info is not None:
        ...
        session_shape_dict = get_dims_for_loader_dict(dataloaders)
        n_neurons_dict = {k: v[out_name][1] for k, v in session_shape_dict.items()}
        in_shapes_dict = {k: torch.Size([v[in_name][0], 3, v[in_name][2], v[in_name][2]]) for k, v in session_shape_dict.items()}     #for saliency
        in_shapes_dict2 = {k: torch.Size([v[in_name][0], 1, v[in_name][2], v[in_name][2]]) for k, v in session_shape_dict.items()}     # for images
    ...
    readout = SaliencyShifter(core, in_shapes_dict=in_shapes_dict, in_shapes_dict2=in_shapes_dict2, n_neurons_dict=n_neurons_dict,
