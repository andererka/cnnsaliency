...
from .utility import get_validation_split, get_cached_loader, get_fraction_of_training_images, get_crop_from_stimulus_location  #ImageCache
from nnfabrik.utility.nn_helpers import get_module_output, set_random_seed, get_dims_for_loader_dict
from nnfabrik.utility.dj_helpers import make_hash



def conca_im(cache1, cache2, path):
        
    assert cache1.__len__() == cache2.__len__()
    for i in range(1, cache1.__len__()+1):
        img1 = []
        img2 = []
        img1 = cache1.__getitem__(i)
        img2 = cache2.__getitem__(i)

        image = np.concatenate((img1, img2),
                                      axis=0)
            
        key = make_hash(i)
        cache3 = ImageCache(path=path, transform=False, normalize=False)
        
        cache3.cache[key] = image
        return cache3
...
def monkey_saliency_loader(dataset,
    ...
    if os.path.exists(stats_path):
        ...
        # Initialize cache
        cache = ImageCache(path=image_cache_path, sal_path = saliency_cache_path, subsample=subsample, crop=crop, scale=scale, img_mean= img_mean, img_std=img_std, transform=True, normalize=True)
    
    else: #if stats not given
        # Initialize cache with no normalization
        cache = ImageCache(path=image_cache_path,sal_path =saliency_cache_path, subsample=subsample, crop=crop, scale=scale, transform=True, normalize=False) 
...
class ImageCache:
    ...
    def __init__(self, path=None, sal_path = None, subsample=1, crop=0, scale=1, img_mean=None, img_std=None, transform=True, normalize=True, filename_precision=6):
