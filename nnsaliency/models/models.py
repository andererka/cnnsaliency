...
from .readouts import MultipleFullGaussian2d, MultiReadout, MultipleSpatialXFeatureLinear, MultipleRemappedGaussian2d, \
    MultipleGaussian2d, MultipleAttention2d, SaliencyShifter
...
def se_core_saliency_shifted_readout(dataloaders, seed, hidden_channels=32, input_kern=13,  # core args
                                     hidden_kern=3, layers=3, gamma_input=15.5,
                                     skip=0, final_nonlinearity=True, momentum=0.9,
                                     pad_input=False, batch_norm=True, hidden_dilation=1,
                                     laplace_padding=None, input_regularizer='LaplaceL2norm',
    ...
    Args:
        ...
        added args:
        use_saliency: Default True. If set to False, then pixel values of saliency maps will be all set to one.
        use_images_for_remapper: Default False. If True, the images will be used for calculating the shifts per neuron.
                                In this case, the argument "remapper_channels" should be set to one.
        remapper_channels: defines the number of channels for the remapper part.
    ...
    if data_info is not None:
        ...
        session_shape_dict = get_dims_for_loader_dict(dataloaders)
        n_neurons_dict = {k: v[out_name][1] for k, v in session_shape_dict.items()}
        in_shapes_dict = {k: torch.Size([v[in_name][0], remapper_channels, v[in_name][2], v[in_name][2]]) for k, v in
                          session_shape_dict.items()}  # for saliency
        in_shapes_dict2 = {k: torch.Size([v[in_name][0], 1, v[in_name][2], v[in_name][2]]) for k, v in
                           session_shape_dict.items()}  # for images

    set_random_seed(seed)

    core = SE2dCore(input_channels=1,
                    hidden_channels=hidden_channels,
                    input_kern=input_kern,
                    hidden_kern=hidden_kern,
                    layers=layers,
    ...
    readout = SaliencyShifter(core, in_shapes_dict=in_shapes_dict, in_shapes_dict2=in_shapes_dict2,
                              n_neurons_dict=n_neurons_dict,
                              init_mu_range=init_mu_range, bias=readout_bias,
    ...
    class Encoder(nn.Module):
        ...
        def forward(self, x, data_key=None, **kwargs):
            ...
            ### option to test remapper with images as input and not saliency maps
            if (use_images_for_remapper == True):
                sal = x[:, 0:1, :, :].repeat(1,remapper_channels,1,1).cuda()

            sal = sal.cuda()

            sample = kwargs["sample"] if 'sample' in kwargs else None

            ###takes sal and img in forward method of RemappedGaussian2d_Sal
            x = self.readout(x=img, sal=sal, data_key=data_key, sample=sample)
...
def simple_core_transfer(dataloaders,
    ...
    if readout_transfer_key:
        readout_state = (readout_transfer_table & readout_transfer_key).get_full_config(include_state_dict=True)[
            "state_dict"]
...
def se_core_shared_gaussian_readout(dataloaders,
    ...
    if key is not None:
        ...
        dataloaders, model = TrainedModel().load_model(dict(model_hash=model_hash,
                                                            ...
                                                            trainer_hash=trainer_hash, seed=seed),
...
def augmented_full_readout(dataloaders=None,
    ...
    for data_key, readout in model.readout.items():
        ...
        for i in range(readout.outdims - (32 if mua_in else 0)):
            ...
            model.readout["augmentation"].features.data[:, :, :, insert_index:insert_index + neuron_repeats] = features[
                                                                                                               :, :, :,
                                                                                                               None]
            model.readout["augmentation"].bias.data[insert_index:insert_index + neuron_repeats] = bias
...
def stacked2d_core_dn_linear_readout(dataloaders, seed, hidden_channels=32, input_kern=13,  # core args
                                     ...
                                     readout_bias=False, normalize=True, init_noise=1e-3, constrain_pos=False,
                                     # readout args,
    ...
    class Encoder(nn.Module):
        ...
        def regularizer(self, data_key):
            if self.nonlin is None:
                ...
                return self.core.regularizer() + self.readout.regularizer(data_key=data_key) + self.nonlin.regularizer(
                    data_key=data_key)
